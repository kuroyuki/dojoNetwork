...
-export([my_function/1]).  %to enable spawning
...
start(PortNo) when integer(PortNo) ->
  start(PortNo, {?MODULE, my_function}).   %will spawn getty:my_function/1 later
...
my_function(Value)->
  receive
    {Sender, {data, Data}}  ->
      io:format("My function received bytes from remote ~p~n", [Data]),
      Sender!{self(), {command, "reply \n\r"}},
      my_function(Value);
    Any ->
      io:format("My function received unknown message ~p~n", [Any]),
      my_function(Value)
  end.
